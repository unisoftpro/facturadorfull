  ...
  >
    <form autocomplete="off" @submit.prevent="clickAddItem">
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group" :class="{ 'has-danger': errors.item_id }">
              <label class="control-label">
                Producto/Servicio
                <a href="#" @click.prevent="showDialogNewItem = true"
                  >[+ Nuevo]</a
                >
              </label>
          ...
          <div class="col-md-3">
            <div class="form-group" :class="{ 'has-danger': errors.quantity }">
              <label class="control-label"
                >Cantidad {{ purchaseOrderTypeId }}</label
              >
              <el-input-number
          ...
          <div class="col-md-3">
            <div
              class="form-group"
              :class="{ 'has-danger': errors.previous_cost }"
            >
              <label class="control-label">Costo anterior</label>
              <el-input v-model="form.previous_cost" readonly>
                <template
                  slot="prepend"
                  v-if="form.previous_currency_type_id"
                  >{{ form.previous_currency_type_id }}</template
                >
              </el-input>
              <small
                class="form-control-feedback"
                v-if="errors.previous_cost"
                v-text="errors.previous_cost[0]"
              ></small>
            </div>
          </div>
          ...
          <div class="col-md-3">
            <div
              class="form-group"
              :class="{ 'has-danger': errors.attended_quantity }"
            >
              <label class="control-label">Cantidad Atendida</label>
              <el-input-number
                v-model="form.attended_quantity"
                :min="0"
                :max="form.quantity"
              ></el-input-number>
              <small
                class="form-control-feedback"
                v-if="errors.attended_quantity"
                v-text="errors.attended_quantity[0]"
              ></small>
            </div>
          </div>
            ...
            >
              ...
              <div class="card-body px-0 pt-2" style="display: none">
                <div class="col-md-12 px-0" v-if="discount_types.length > 0">
                  <label class="control-label">
                    Descuentos
                    <a href="#" @click.prevent="clickAddDiscount"
                      >[+ Agregar]</a
                    >
                  </label>
                  <table class="table">
                      ...
                      <tr v-for="(row, index) in form.charges">
                        ...
                        <td>
                          <button
                            type="button"
                            class="btn btn-danger"
                            @click.prevent="clickRemoveCharge(index)"
                          >
                            x
                          </button>
                        </td>
                ...
                <div class="col-md-12 px-0" v-if="attribute_types.length > 0">
                  <label class="control-label">
                    Atributos
                    <a href="#" @click.prevent="clickAddAttribute"
                      >[+ Agregar]</a
                    >
                  </label>
                  <table class="table">
      ...
      <div class="form-actions text-right pt-2">
        ...
        <el-button type="primary" native-type="submit" v-if="form.item_id"
          >Agregar</el-button
        >
      </div>
    </form>
...
export default {
  ...
  methods: {
    ...
    getPreviousCost() {
      this.$http
        .get(`/${this.resource}/item/previous-cost/${this.form.item_id}`)
        .then((response) => {
          this.form.previous_cost = response.data.previous_cost;
          this.form.previous_currency_type_id =
            response.data.previous_currency_type_id;
        });
    },
    clickAddItem() {
