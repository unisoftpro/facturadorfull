...
export default {
  ...
  methods: {
    ...
    getListPrice() {
      this.$http
        .get(`/${this.resource}/item/list-price/${this.form.item_id}`)
        .then((response) => {
          var currencytype =  response.data.currentype;
          let listprice = response.data.list_price;

          if(currencytype==="USD"){

              this.form.list_price =_.round(listprice * this.exchange_rate_sale,2);
          }else{
