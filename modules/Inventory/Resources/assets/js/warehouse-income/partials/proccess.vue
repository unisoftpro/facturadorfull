    ...
    <div class="row">
      <div class="col-lg-6 col-md-6 col-sm-6 text-center font-weight-bold mt-3">
        <el-popover placement="right" width="350" trigger="click">
          <div style="text-align: left; margin: 0">
            ...
            <el-select v-model="form_process_price.currency_type_id" filterable>
              <el-option
            ...
            <el-input v-model="exchange_rate_sale"></el-input>
            <br />
            <br />
            <el-button @click="clickProccessExterior" type="primary" plain
              >Procesar</el-button
            >
          </div>

          <el-button type="warning" icon="el-icon-s-tools" slot="reference"
      ...
      <div class="col-lg-6 col-md-6 col-sm-6 text-center font-weight-bold mt-3">
        <el-popover placement="right" width="350" trigger="click">
          <div style="text-align: left; margin: 0">
            <label for="">Moneda</label>
            <el-select v-model="form_process_local.currency_type_id" filterable>
              <el-option
                v-for="option in currency_types"
                :key="option.id"
                :value="option.id"
            ...
            <br />
            <br />
            <el-button @click="clickProccessLocal" type="primary" plain
              >Procesar</el-button
            >
          </div>
          <el-button type="warning" icon="el-icon-s-tools" slot="reference"
...
export default {
  ...
  methods: {
    async getTipoCambio() {
      this.searchExchangeRateByDate(this.date_of_issue).then((response) => {
        this.exchange_rate_sale = response;
      });
    },
    clickDownload(template) {
      window.open(
        `/${this.resource}/download/${this.form.external_id}/${template}`,
        "_blank"
      );
    },
    clickPrint(format) {
    ...
    async getItemPurchaseExterior() {
      ...
      await this.$http
        .get(`/${this.resource}/items/${this.recordId}`)
        .then((response) => {
    ...
    calculateValuesExterior(row, factor, currency_type_id_new) {
