	...
	private function getStructureAdsoft($documents)
	{
		...
		foreach ($documents as $row) {
			$rows[] = [
				...
				'tipdoc'     => intval($row->document_type_id) * 1,
				'tipmon'     => strtoupper($row->currency_type->description),
				'detrac'     => '',
				'imp_vta'    => $row->state_type_id == '11' ? 0 : number_format($row->total_value, 2, '.', ''),
				'isc'        => $row->state_type_id == '11' ? 0 : number_format($row->total_isc, 2, '.', ''),
				'icbper'     => $row->total_plastic_bag_taxes > 0 ? number_format($row->total_plastic_bag_taxes, 2, '.', '') : '',
				'imp_ina'    => 0,
				'imp_exo'    => $row->state_type_id == '11' ? 0 : number_format($row->total_value, 2, '.', ''),
				'imp_exp'    => '',
				'recargo'    => '',
				'imp_igv'    => number_format($row->total_igv, 2, '.', ''),
				...
				'pasaporte'  => '',
				'cta_vta'    => '',
				'tip_cam'    => number_format($row->exchange_rate_sale, 2, '.', ''),
			];
		}
